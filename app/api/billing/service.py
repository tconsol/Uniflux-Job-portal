...
from app.database import subscriptions, plans, users
...
async def get_all_plans() -> list[Dict[Any, Any]]:
    return await plans.find({"is_active": True}).to_list(length=None)


async def get_plan_by_slug(slug: str) -> Optional[Dict[Any, Any]]:
    return await plans.find_one({"slug": slug})
...
async def create_stripe_checkout_session(user_id: str, plan_slug: str, success_url: str, cancel_url: str) -> tuple[str, str]:
    ...
    if not stripe_customer_id:
        ...
        await users.update_one({"_id": user_id}, {"$set": {"stripe_customer_id": stripe_customer_id}})

    session = stripe.Checkout.Session.create(
        customer=stripe_customer_id,
        payment_method_types=["card"],
        line_items=[{"price": plan.get("stripe_price_id_monthly"), "quantity": 1}],
        mode="subscription",
...
async def handle_subscription_deleted(stripe_sub: dict):
    ...
    await subscriptions.update_one(
        {"stripe_subscription_id": subscription_id},
        {"$set": {"plan": "basic", "status": "expired", "updated_at": datetime.utcnow()}}
    )
    await generate_daily_job_set("basic")


async def cancel_subscription(user_id: str) -> bool:
    subscription = await subscriptions.find_one({"user_id": user_id, "status": "active"})
    ...
    await subscriptions.update_one(
        ...
        {"$set": {"plan": "basic", "status": "canceled", "updated_at": datetime.utcnow()}}
    )
    await generate_daily_job_set("basic")
    return True